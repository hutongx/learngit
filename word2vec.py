...
def naiveSoftmaxLossAndGradient(
    ...
    z = np.dot(outsideVectors, centerWordVec) # N x M x M x 1 ==> N x 1

    y_hat = softmax(z) # N x 1
...
def negSamplingLossAndGradient(
    ...
    # gradCenterVec = (allScores[0] - 1) * allScores[0] + (1 - sigmoid(-allScores[1:])).dot(allVecs[1:])
    ...
    loss = - np.log(z) # 目标 y 的损失  part of loss: 公式第一部分

    gradOutsideVecs[outsideWordIdx] += centerWordVec * (z - 1.0) # u_o
    gradCenterVec += outsideVectors[outsideWordIdx] * (z - 1.0) # part of u_c
    # Implementation with for loop
    # for k in range(K):
    #     samp = indices[k+1] # 第0个是outsideWordIdx
    #     z = sigmoid(np.dot(-outsideVectors[samp], centerWordVec))
    #     loss -= np.log(z)
    #     gradOutsideVecs[samp] -= centerWordVec * (z - 1.0) # u_k
    #     gradCenterVec -= outsideVectors[samp] * (z - 1.0)  # the rest of u_c
    # Vectorized implementation
    u_k = outsideVectors[negSampleWordIndices]
    z = sigmoid(-np.dot(u_k, centerWordVec))
    loss += np.sum(- np.log(z)) # the rest part of loss: 公式第二部分
    gradCenterVec += np.dot((z-1), u_k) * (-1) # the rest of u_c
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    word2vecLossAndGradient -- the loss and gradient function for
                               a prediction vector given the outsideWordIdx  注意这里, 可以是上面我们 完成的两个损失函数 之一, 这样我们就可以匹配参数.
